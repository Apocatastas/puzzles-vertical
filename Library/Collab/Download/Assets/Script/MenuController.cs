    ...
    public class MenuController : MonoBehaviour
    {
        ...
        public IEnumerator SplashRoutine()
        {
            ...
            if (PlayerPrefs.GetInt("OTHELLOSHEN") == 0)
            {
                ...
                Amplitude.Instance.logEvent("Onboarding_Questions_Open");
                childName.text = "";
                down.interactable = false;
                if (ArrowCheck()) { arrow.interactable = true; } else { arrow.interactable = false; }
        ...
        public void CloseQuestionnaire()
        {
            if (ArrowCheck())
            {
                Amplitude.Instance.logEvent("Onboarding_Questions_Arrow");
                Amplitude.Instance.logEvent("onboarding_PC");
                parentInterface.SetActive(true);
                uiCanvas.SetActive(true);
                PlayerPrefs.SetInt("OTHELLOSHEN", 1);
                questInterface.SetActive(false);


                Dictionary<string, object> demoOptions6 = new Dictionary<string, object>() { { "Onboarding_Questions_Name", childName.text } };
                Amplitude.Instance.logEvent("Onboarding_Questions_Name", demoOptions6);

                Dictionary<string, object> demoOptions7 = new Dictionary<string, object>() { { "Onboarding_Questions_Age", childAge.text } };
                Amplitude.Instance.logEvent("Onboarding_Questions_Age", demoOptions7);

                if (female.isOn)
                {
                    Dictionary<string, object> demoOptions8 = new Dictionary<string, object>() { { "Onboarding_Questions_Gender", "female" } };
                    Amplitude.Instance.logEvent("Onboarding_Questions_Gender", demoOptions8);
                }

                if (male.isOn)
                {
                    Dictionary<string, object> demoOptions8 = new Dictionary<string, object>() { { "Onboarding_Questions_Gender", "male" } };
                    Amplitude.Instance.logEvent("Onboarding_Questions_Gender", demoOptions8);
                }
        ...
        {
            return (int.Parse(childAge.text) > 0) && (!(childName.text == "")) && (male.isOn || female.isOn);
        ...
        public void SwitchGender(Toggle who)
        {
            switch (who.name)
            {
                case "Female":
        ...
        public void AgeUp()
        {
            int current = int.Parse(childAge.text);
            if ( current < 1)
            {
                down.interactable = true;
            }
            childAge.text = (current + 1).ToString();
            if (ArrowCheck()) { arrow.interactable = true; } else { arrow.interactable = false; }
        }
